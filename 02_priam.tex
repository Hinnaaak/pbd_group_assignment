...
% Figure on risk level assessment

The risk levels for a given scenario is then calculated in the following
way\cite{de2016priam}:

\begin{enumerate}
  \item{Find the value of \textit{exploitability} for each leaf node in the harm tree.}
       ...
       relevant system attributes. For illustrative purposes, both are in this
       case assigned the value medium.

      \begin{figure}[hbt!]
        \centering
        \includegraphics[width=\textwidth]{pictures/exploitability.png}
        \caption{}{Harm tree example.}
        \label{fig:harm_tree_ex}
      \end{figure}

  \item{For each exploitation, choose the values of the relevant attributes of
        ...
        the majority of attributes does not favor an exploitation, while the opposite is
        true for the \textit{Hacker}.

  \item{Find out the likelihood of each of these exploitations from the above
        exploitability value and values of the relevant risk source attributes}

        % Measurement rule

        The measurement rule to determine the likelihood of exploitation can be
        seen in Figure ? above, which means that the likelihood of our chosen exploitations are \textit{Limited} and \textit{Significant} respectively.
